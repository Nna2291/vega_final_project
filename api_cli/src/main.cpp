...
int open_fifo_for_writing(const std::string& path) {
    ...
    // Открываем на запись; вызов будет блокировать, пока кто-нибудь не откроет FIFO на чтение.
...
int main() {
    ...
    PricePipe pipe;
    auto provider = std::make_shared<MoexClient>();
    ...
    // Consumer loop: читаем из внутреннего PricePipe и отправляем JSON-строки в именованный канал.
    PriceUpdate update;

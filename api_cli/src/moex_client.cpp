...
std::string MoexClient::build_url(const std::string& ticker) const {
    ...
    // We request both securities and marketdata; meta is disabled for compactness.
...
PriceUpdate MoexClient::parse_update_from_json(const std::string& body,
    ...
    // SYSTIME is a quoted string: "YYYY-MM-DD HH:MM:SS"
    ...
    // Interpret as UTC for consistency; DB/analytics can adjust timezone if needed.
